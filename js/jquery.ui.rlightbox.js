 ...
 *
 * Copyright 2011 Wojciech â€˜rrhâ€™ Ryrych
...
$.widget( "ui.rlightbox", {
	options: {
		...
		videoWidth: 640,
		videoHeight: 385,
		errorMessage: "Oh dear! Something went wrong! If the problem still appears let the pageâ€™s admin know. Would you like to try again or reject the content?",
	...
	_create: function() {
		...
		if ( _setElement.type !== undefined )Â {
...
// its initial DOMÂ structure it transformed into the plugin's one. Therefore
// â€˜thisâ€™ always refers to the plugin instance.
...
// options and _create) and created only one UI, the UI would have â€˜thisâ€™ scope
...
// set of options, â€˜this.optionsâ€™ would always refer to the first instance.
// Since in rlightbox we donâ€™t operate on elements directly but only get their
// urls, all methods are placed in the extended scope of the plugin:
// â€˜$.ui.rlightbox.globalâ€™. An instance â€˜thisâ€™ scope is merely used when we
// use options.
$.extend($.ui.rlightbox, {
	global: {
		addToSet: function( setElement ) {
			...
			var _setName = this.getSetName( setElement.self ),
				...
				_class = "." + _setPrefix + "_" +Â _setName,
		...
		checkButtonsState: function()Â {
			...
			if (Â _currentSet === "single" || _totalElements === 1 )Â {
				this.setButtonState( "disabled" );
			} else if ( _currentElement === 1 && _isLoop === false ) {
				
				// in case of 1st element when loop is disabled
				this.setButtonState(Â "disabled", $lb.prev );
				
				// when there are only two elements in a set
				this.setButtonState(Â "default", $lb.next );
			} else if ( _currentElement === _totalElements && _isLoop === false )Â {
				
				// in case of last element
				this.setButtonState(Â "disabled", $lb.next );
				
				// when there are only two elements in a set
				this.setButtonState(Â "default", $lb.prev );				
			} else {
				
				// between first and last elements or when the loop is enabled
				this.setButtonState(Â "default" );
			}
		},		

		checkMinimalSize: function( size, number )Â {

			// checks if the â€˜numberâ€™ of a given â€˜sizeâ€™ (width, height) is smaller
			// then the minimal lightbox size. If so, it returns the minimal size
			// otherwise it returns the â€˜numberâ€™
			var data = this.data,
		...
		destroy: function()Â {
			...
			// from the widget factory but we canâ€™t call it because it acts upon
		...
		extractAnchor: function( thisElement ) {
			...
			// $element keeps jQuery object of an anchor and itâ€™s used for example
					...
					$.each(content.urls,
						function( index, regExp ) {
							...
							if ( _res !== null ) {
								...
								if ( content.type === "image" || content.type === "flash" || thisElement.options.overwriteTitle )Â {
									_result.title = $anchor.attr(Â "title"Â );
									_result.url = _url;
								}
		...
		getAvailableScreenSize: function() {
			...
			return {
				width: this.getWindowSize(Â "width"Â ) - _padding,
				height: this.getWindowSize(Â "height"Â ) - data.headerHeight - _padding
			};
		},		
		...
		getImageStatus: function( width, height ) {
			...
			var _statusWidth, _statusHeight,
				...
				_windowWidth = this.getWindowSize(Â "width"Â ),
				_windowHeight = this.getWindowSize(Â "height" ),
		...
		getLightbox: function()Â {
			...
			if ( !$lb.root )Â {
				...
				// create the DOMÂ structure of the lightbox
				...
				$lb.close.add( $lb.overlay ).click( $.proxy(this.closeLightbox, this)Â );
				
				// goes to the next element when button is clicked
				$lb.next.click( $.proxy(this.next, this)Â );
				
				// and goes to the prev element when prev button is clicked
				$lb.prev.click( $.proxy(this.prev, this)Â );			
				...
				$lb.next
					...
					.hover(
						function()Â {
							if ( $(this).is(":not(.ui-state-disabled)")Â ) {
								self.setButtonState( "highlight", $(this) );
							}
						},
						function()Â {
							if ( $(this).is(":not(.ui-state-disabled)") )Â {
								self.setButtonState(Â "default", $(this) );								
							}
						}
					);		
				...
				$lb.contentContainer
					.mousemove( $.proxy(this.showArrow, this)Â )
					.mousemove( $.proxy(this.checkSide, this) )
					.mousemove( $.proxy(this.setCursor, this)Â )
					.click(
						function()Â {
							if ( data.sideÂ === "left" )Â {
					...
					.mousedown( $.proxy(this.panoramaStart, this)Â )
				...
				$lb.panoramaIcon
					.click( $.proxy(this.panoramaToggle, this)Â )
					.hover( $.proxy(this.panoramaHighlight, this)Â );

				// resize lightbox when window size changes
				$( window ).bind( "resize.rlightbox", $.proxy(this.liveResize, this)Â );

				// keyboard navigation
				$( document ).keyup( $.proxy(this.handleKeyboard, this)Â );			
			}
		},

		getOptimalSize: function( size, numberÂ )Â {
			...
			if ( size === "width"Â )Â {
				...
				if ( number < _minimalLightboxHeight ) {
					return _minimalLightboxHeight;
				} else if ( number > _screenHeightÂ )Â {
		...
		getParam: function(Â param, urlÂ )Â {

			// with param â€˜withâ€™ and url â€˜foo.flv?width=100" it returns â€˜100â€™
			var _result,
				_regExpString = "[\\?&]" + param + "=(\\w+)",
				_regExp = new RegExp(Â _regExpString );

			_result = _regExp.exec( urlÂ );
		...
		getSetName: function( thisElement ) {
			...
			// if an anchor has class of e.g. â€˜lb_galleryâ€™ _getSetName() returns â€˜galleryâ€™ string as a set
				...
				if ( ((_statusWidth === 1 || _statusWidth === -1) && _statusHeight !== 2) && ((_statusHeight === 1 || _statusHeight === -1) && _statusWidth !== 2) ) {
					...
					if ( _statusWidth === 1 || _statusWidth === -1 ) {
						...
						if ( w > h ) {
							...
							if ( _imageTargetHeight <= _minimalLightboxHeight || _lightboxTargetHeight + _lightboxPadding +Â _headerHeight > _windowHeight ) {
								_calculateSizes( _imageTargetWidth, _imageTargetHeight );
							}
						} else {
							_lightboxTargetHeight = _windowHeight - _headerHeight - _lightboxPadding;
							_imageTargetHeight = _lightboxTargetHeight;							
							_heightRatio = _lightboxTargetHeight / h;
							_lightboxTargetWidth = Math.ceil(Â w * _heightRatio ) - _lightboxPadding;
							_imageTargetWidth = _lightboxTargetWidth;
		...
		getWindowSize: function( sizeÂ )Â {
			var data = this.data,
				_windowWidth = $( window ).width(),
				_windowHeight = $( windowÂ ).height(),
				_minimalLightboxSize = data.minimalLightboxSize,
				_lightboxPadding = data.lightboxPadding,
				_headerHeight = data.headerHeight,
			...
			if ( size ===Â "width"Â )Â {
				if ( _windowWidthÂ < _minimalLightboxWidth ) {
					return _minimalLightboxWidth;
				} else {
					return _windowWidth;
				}
			} else {
				if ( _windowHeight < _minimalLightboxHeight )Â {
					return _minimalLightboxHeight;
				} else {
		...
		handleKeyboard: function( event )Â {
			var data = this.data,
				_currentElement = data.currentSetElement,
		...
		liveResize: function() {
			...
			if ( data.ready && data.panoramaOn === false && _elementType === "image" ) {
				...
				this.panoramaShrink();
				this.panoramaCheckAvailability();
			} else if ( data.ready && _elementType !== "image"Â )Â {
				this.queueCenterLightbox();
			}
		},

		loadContentFlash: function( urlÂ )Â {
			...
			function _load()Â {
				...
				// if any exist; â€˜inlineâ€™ width and height overwrite that of options
				_width = self.getParam(Â "width", urlÂ );
				_height = self.getParam(Â "height", urlÂ );

				// if &width and &height are invalid, use a default one
				if ( _width === null || isNaN(_width) )Â {
					_width = _options.videoWidth;
				}

				if (Â _height === null || isNaN(_height) )Â {
				...
				// we have to add â€˜widthâ€™ and â€˜heightâ€™ to the $contentWrapper
				// explicitly since browsers canâ€™t inherit them
				$contentWrapper = $(Â "<div></div>"Â );
				$contentWrapper.css(
					{
						display: "none",
						width: _width,
				...
				// add structure
				$content
					.removeClass(Â "ui-lightbox-loader" )
					.empty()
					.append( _structure )
					.children()
						.wrap(Â $contentWrapper );
				
				_dfd.resolve();
			}

			// delay â€˜_loadâ€™ because we have to return promise
			setTimeout( _load, 1000 );

			return _dfd.promise();
		},

		loadContentImage: function( url )Â {
			var $lb = this.$lightbox,
				data = this.data,
				self = this,
				_currentElement = data.currentSetElement,
			...
			$newImage
				...
				.error(
					function()Â {
						$lb.content.removeClass(Â "ui-lightbox-loader"Â );
		...
		loadContentVimeo: function( url ) {
			...
			// show loader
			$lb.content.addClass(Â "ui-lightbox-loader"Â );
			...
			.success(
				function( data ) {
					...
					$lb.content
						...
						.append( data.html )
						.children()
							.wrap(Â "<div style='display: none'></div>" )
							.end()
						.find(Â "div:first"Â )
							.width( data.width )
							.height( data.height );							
					...
					if ( _options.overwriteTitle === false )Â {
			...
			.error(function() {
				$lb.content.removeClass(Â "ui-lightbox-loader" );
				self.showErrorMessage();
		...
		navigationGoToElement: function( number )Â {
			...
			this.setNextQueue();
			$lb.queueContainer.next.dequeue(Â "lightboxNext" );
		},		
		...
		next: function()Â {
			...
			if ( _isReady && _set !== "single" && _isPanoramaOn === false ) {
				...
				if (Â _play)Â {
					...
					// next element - trigger the queue â€˜nextâ€™ - first update it
					this.setNextQueue();
		...
		open: function( thisElement ) {
			var data = this.data,
				...
				_currentUrl = $( _jqElement ).attr(Â "href" );
			...
			// keep a reference to a current element in a setÂ (consisting of a url, typeâ€¦)
		...
		panoramaCheckAvailability: function()Â {
			...
			// Panoramaâ„¢ is enabled only when the image canâ€™t be displayed in its
			// orignal size.
			var data = this.data,
				_currentElement = data.currentSetElement,
			...
			if ( _currentImageWidth < _originalImageWidth || _currentImageHeight < _originalImageHeightÂ )Â {
				this.panoramaShowIcon(Â "expand"Â );
				
				// cuz we donâ€™t want to check it twice in panoramaToggle()
		...
		panoramaExpand: function( event ) {
			...
			$lb.content.css(Â "overflow", "hidden" );
			...
			// enlarge lightboxâ€™s content to fit the screen best
			...
			// cursor was â€˜pointerâ€™, [Z] buttons was pressed (â€˜defaultâ€™ cursor)
			// [Z] was pressed again â†’ cursor is still â€˜pointerâ€™
			this.setCursor();
		},

		panoramaHideIcon: function()Â {
			var data = this.data,
				$lb = this.$lightbox;
		...
		panoramaHighlight: function()Â {
			...
			$lb.panoramaIcon.attr("class",
				function(index, oldValue) {
					_suffixPosition = oldValue.indexOf( _suffixÂ );
					if ( _suffixPosition !== -1Â ) {
		...
		panoramaShowIcon: function( icon, state )Â {
			var $lb = this.$lightbox,
				_state = state || "",
		...
		panoramaShrink: function( event ) {
			...
			// the icon we lose focus and state end up with normal state
			// not when key is pressed
			if ( eventÂ && event.type === "click"Â )Â {
				this.panoramaShowIcon( "expand", "-hover"Â );
			} else {
			...
			// fixes issue with Panorama in Firefox 3.0, 3.5, 3.6
			$lb.content.css(Â "overflow", "visible" );			
		...
		panoramaStop: function( event ) {
			...
			if ( data.panoramaOn ) {
				...
				// subtrack 1 so that the viewport doesnâ€™t overlap the map border
				$lb.viewport.css({
		...
		panoramaToggle: function( event ) {
			...
			// we couldnâ€™t use .toggle( expand, shrink ) on panorama icon because when lb is closed after panorama was turned on (we were in the panorama mode)
			// and open again and next image once again can be zoomed we need to make sure that
			// expand is the first action â€“ using jQuery .toggle() â€˜expandâ€™ would be the fist action again (because of its internal queue)
			var data = this.data,
		...
		removeSetElement: function( number )Â {
			...
			if( _total === 1 || _currentSet === "single" )Â {
				this.closeLightbox();

				// remove the instance from encapsulated DOMÂ element (jquery one)
				this.destroy();
			} else {
				this.destroy();
		...
		replaceHtmlPatterns: function( htmlString, patternsÂ )Â {

			// replaces patterns like {width} used in html e.g in data.htmlFlash
			// with real data given in patterns object
			$.each(patterns, function(key, value)Â {
				var _regExp = new RegExp( "{"Â + key +Â "}", "g" );
		...
		setButtonState: function( state, jqElement )Â {
			var $lb = this.$lightbox,
			...
			switch ( state )Â {
				...
				case "highlight":
					jqElem.addClass(Â "ui-state-highlight"Â );
					break;
				
				case "disabled":
					jqElem.addClass(Â "ui-state-disabled"Â );
					break;
			}
		},
		
		setCursor: function( event )Â {
			var data = this.data,
				$lb = this.$lightbox,
			...
			if ( data.readyÂ )Â {
				if (Â (_currentSet === "single" || _totalElements === 1 || _currentElement === 1 && _side ===Â "left" || _currentElement === _totalElements && _side === "right") && _panoramaEnabled === false && (_setElementType === "image" || (_setElementType !== "image" &&Â _isError)) )Â {

					// single element or single element in a named set or first element in a set or last element in a set
					// WHEN panorama is DISABLED, and when element type is â€˜imageâ€™ or the Error Screen is shown
					// and when loop is DISABLED
					if (Â _isLoop === false )Â {
						$contentContainer.css(Â "cursor", "default" );						
					} else {
						
						// otherwise show â€˜pointerâ€™ in cases mentioned above
						$contentContainer.css(Â "cursor", "pointer" );						
					}

				} else if ( _panoramaEnabled )Â {

					// panorama is enabled
					$contentContainer.css(Â "cursor", "move" );
				} else if ( _setElementType === "image" ||Â (_setElementType !== "image"Â &&Â _isError) ) {
					...
					// for flash videos
					$contentContainer.css(Â "cursor", "auto" );
				}
			} else {
				$contentContainer.css(Â "cursor", "default" );
			}
		...
		setOpenQueue: function() {
			...
			// half of the operations are the same - they ovelap, and the rest such as â€˜show the overlayâ€™, â€˜center lightboxâ€™,
			// â€˜slide up the headerâ€™ and â€˜hide contentâ€™ are run only in one queue not in both
			// thus to not repeat oneself we keep in the queue lists only references to these methods
			// each one of these methods (that begin with _queueâ€¦) are passed â€˜nextâ€™ parameter that is a reference to another
			// method in the queue.
			// $proxy is needed to have an access to a â€˜globalâ€™ scope of the plugin â€“ every method that is called in the queue
		...
		setReferences: function() {
			...
			$lb.arrow = $lb.contentContainer.find(Â "#ui-lightbox-arrow" );
			$lb.header = $lb.root.find( "#ui-lightbox-bottombar" );
			$lb.headerWrapper = $lb.header.find(Â "#ui-lightbox-title-wrapper"Â );		
			$lb.overlay = $( "#ui-lightbox-overlay" );
			$lb.next = $lb.root.find( "#ui-lightbox-button-next" );
			$lb.prev = $lb.root.find(Â "#ui-lightbox-button-prev" );
			//$lb.play = $lb.root.find(Â "#ui-lightbox-button-play" );
			$lb.controlButtons = $lb.next.add( $lb.prev );//.add( $lb.playÂ );
		...
		showErrorMessage: function()Â {
			...
			$structure = $(Â _structureÂ );

			$again = $structure.find( "#ui-lightbox-error-footer-again" );
			$reject = $structure.find(Â "#ui-lightbox-error-footer-reject"Â );
			// â€˜againâ€™ button give a user a chance to try loading content again
			$again
				.click(function()Â {
					self.navigationGoToElement(Â _currentElementNumber );
				})
				.hover(
					function()Â {
						$( this ).toggleClass(Â "ui-state-hover"Â );
					}
				);
			...
			$reject
				.click(function()Â {
					self.removeSetElement(Â _currentElementNumberÂ );
				})
				.hover(
					function()Â {
						$( this ).toggleClass(Â "ui-state-hover"Â );
			...
			$lb.content
				.empty()
				.append( $structureÂ )
				.find(Â "#ui-lightbox-error"Â )
					.width(Â _errorScreenWidth )
					.height(Â _errorScreenHeight )
					.end()
				.children()
					.hide();

			// because we donâ€™t want to break the animation queue we need to tell
		...
		queueCenterLightbox: function( next ) {
			var $lb = this.$lightbox,
				$root = $lb.root,
				_screenWidth = this.getWindowSize(Â "width"Â ),
				_screenHeight = this.getWindowSize(Â "height"Â ),
		...
		queueLoadContent: function( next ) {
			...
			switch ( _currentSetElement.type ) {
				...
				case "flash":
					_loadContentMethod =Â "loadContentFlash";
			}

			$.when( this[_loadContentMethod](_currentSetElement.url) ).then(function()Â {
				next();
			});
		},
		...
		queueResizeLightbox: function( next ) {
			...
			if ( _currentElement.type === "image" &&Â _isError === false )Â {
				...
				// if you use this method in the context of a queue then use animation; otherwise when used in live resize, donâ€™t animate it
				if ( $.isFunction(next) ) {
					_speed = _options.animationSpeed;
				} else {
					_speed = 0;
			...
			} else if ( (_currentElement.type === "youtube" || _currentElement.type === "vimeo")Â && _isError === false ){

				// do not let lightbox size be smaller than the minimal one
				_lightboxTargetWidth = this.checkMinimalSize( "width", _currentElement.widthÂ );
				_lightboxTargetHeight = this.checkMinimalSize(Â "height", _currentElement.heightÂ );
			} else if ( _currentElement.type === "flash" &&Â _isError === false )Â {
				...
				_lightboxTargetHeight = this.getOptimalSize(Â "height", _currentElement.heightÂ );				
			} else if ( _isError ) {
				_speed = 0;
		...
		queueCenterContent: function( next ) {
			...
			// if we donâ€™t run it in the live resize but in the queue
			if ( next ) {
				next();
			}
		},
		...
		queueShowContent: function( next ) {
			...
			$lb.content.children()
				.fadeIn( _options.animationSpeed, function() {
					...
					if ( _currentElement.type === "image" && _isError === false )Â {
		...
		queueSlideDownHeader: function( next ) {
			...
			// if you go from penulimate/second element to the last/first element change cursor to â€˜defaultâ€™
			// must be after â€˜data.ready = trueâ€™!!!
			this.setCursor();
		...
		data: {
			...
			htmlFlash: "" +
				"<object classid='clsid:D27CDB6E-AE6D-11cf-96B8-444553540000' width='{width}' height='{height}'>" +
					"<param name='movie' value='{url}' />" +
						"<!--[if !IE]>-->" +
						"<object type='application/x-shockwave-flash' data='{url}' width='{width}' height='{height}'>"Â +
						"<!--<![endif]-->"Â +
						"<!--[if !IE]>-->" +
						"</object>"Â +
						"<!--<![endif]-->" +
			...
			htmlErrorScreen: "" +
				"<div id='ui-lightbox-error'>" +
					...
					"<div id='ui-lightbox-error-footer'>" +
						"<button aria-disabled='false' role='button' id='ui-lightbox-error-footer-again' class='ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary'>"Â +
							"<span class='ui-button-icon-primary ui-icon ui-icon-refresh'></span>" +
							"<span class='ui-button-text'>{labelAgain}</span>" +
						"</button>" +
						"<button aria-disabled='false' role='button' id='ui-lightbox-error-footer-reject' class='ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary'>"Â +
			...
			htmlLightbox: "" +
				"<div id='ui-lightbox' class='ui-widget ui-widget-content ui-corner-all' style='display: none'>"Â +
					...
					"<div id='ui-lightbox-bottombar' class='ui-widget-header ui-corner-all' style='display: none'>" +
						...
						"</a>" +
						"<div id='ui-lightbox-panorama-icon' style='display: none'></div>"Â +
					"</div>" +
				"</div>",
			htmlMap: "" +
